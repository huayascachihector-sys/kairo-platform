...
import { SAT_SECTIONS } from '../../../lib/examData';
...
export default function SATView({ state, onBack, onStateChange }: Props) {
  const [tab, setTab] = useState(SAT_SECTIONS[0].id);
  const [running, setRunning] = useState<null | 'practica' | 'simulacro'>(null);

  const section = SAT_SECTIONS.find((s) => s.id === tab)!;
  const attempts = useMemo(() => getExamAttempts(state, 'sat'), [state]);
  const sectionAttempts = attempts.filter((a) => a.section === tab);

  const bestPerSection = SAT_SECTIONS.map((s) => {
  ...
  const composite = bestPerSection.every((b) => b > 0)
    ? bestPerSection.reduce((x, y) => x + y, 0)
    : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-2xl shadow-md">🎓</div>
          <div>
            <h1 className="text-2xl font-bold text-surface-900 dark:text-white">SAT</h1>
            <p className="text-surface-500 text-sm">Digital SAT · Reading & Writing y Math</p>
          </div>
        </div>
        <div className="px-5 py-3 rounded-2xl bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 text-center">
          ...
          <p className="text-2xl font-bold text-surface-900 dark:text-white">{composite ?? '—'}<span className="text-sm text-surface-400"> /1600</span></p>
        </div>
      </div>
      ...
      {running ? (
        <QuizRunner
          ...
          onFinish={({ correct, total, minutes }) => {
            ...
            recordExamAttempt({
              exam: 'sat', section: tab, mode: running,
              score: pct, scaledScore: 200 + Math.round(pct * 6),
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mt-4">
              <ScoreChart attempts={sectionAttempts} max={800} label="Puntaje escalado por sección (200–800)" />
            </div>
          </motion.div>
          ...
          <div className="space-y-5">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-xs text-surface-500 mb-4">
                {section.questions.length} preguntas con explicación inmediata.
              </p>
