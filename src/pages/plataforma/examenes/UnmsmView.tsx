...
import { UNMSM_SECTIONS } from '../../../lib/examData';
...
export default function UnmsmView({ state, onBack, onStateChange }: Props) {
  const [tab, setTab] = useState(UNMSM_SECTIONS[0].id);
  const [running, setRunning] = useState<null | 'practica' | 'simulacro'>(null);

  const section = UNMSM_SECTIONS.find((s) => s.id === tab)!;
  const attempts = useMemo(() => getExamAttempts(state, 'unmsm'), [state]);
  const sectionAttempts = attempts.filter((a) => a.section === tab);

  const bestPerSection = UNMSM_SECTIONS.map((s) => {
  ...
  const composite = bestPerSection.every((b) => b > 0)
    ? Math.round((bestPerSection.reduce((x, y) => x + y, 0) / (bestPerSection.length * 100)) * 100)
    : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-2xl shadow-md">🏛️</div>
          <div>
            <h1 className="text-2xl font-bold text-surface-900 dark:text-white">UNMSM</h1>
            <p className="text-surface-500 text-sm">Examen de Admisión UNMSM — 100 preguntas por área con cronómetro oficial</p>
          </div>
        </div>
        <div className="px-5 py-3 rounded-2xl bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 text-center">
          <p className="text-[11px] text-surface-400 font-semibold uppercase tracking-wide">Puntaje est.</p>
          <p className="text-2xl font-bold text-surface-900 dark:text-white">{composite ?? '—'}<span className="text-sm text-surface-400"> /100</span></p>
        </div>
      </div>

      <div className="flex gap-2 p-1 bg-surface-100 dark:bg-surface-800 rounded-xl w-full sm:w-auto sm:inline-flex">
        {UNMSM_SECTIONS.map((s) => (
      ...
      {running ? (
        <QuizRunner
          ...
          onFinish={({ correct, total, minutes }) => {
            ...
            recordExamAttempt({
              exam: 'unmsm', section: tab, mode: running,
              score: pct, scaledScore: pct,
          ...
          <div className="space-y-5">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-xs text-surface-500 mb-4">
                {section.questions.length} preguntas UNMSM actualizadas. Practica por habilidad o área.
              </p>
