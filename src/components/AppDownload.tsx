...
export default function AppDownload() {
  ...
  return (
    <section id="descargar-app" ref={ref} className="py-20 md:py-28 bg-gradient-dark relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight mb-4">
              Estudia desde tu celular <br className="hidden sm:block" />
              o tu laptop. <span className="text-gradient-cyan">Sin límites.</span>
            </h2>
            <p className="text-surface-400 text-lg leading-relaxed mb-8 max-w-lg">
              Instala KAIRO como aplicación en tu dispositivo Android o Windows. Funciona
              igual que la web: tu progreso, sesiones y documentos quedan guardados en tu equipo.
            </p>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-10 max-w-lg">
              {[
                ...
                <div key={f.label} className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/5 border border-white/10 text-center">
            ...
            <p className="text-xs text-surface-500 mt-3 flex items-center gap-1.5">
              <Smartphone className="w-3.5 h-3.5" /> APK para Android · <Monitor className="w-3.5 h-3.5" /> EXE para Windows
            </p>
          </div>
            ...
            >
              ...
              <div className="rounded-[2rem] overflow-hidden border border-white/10 bg-gradient-to-br from-primary-950 via-slate-950 to-accent-950 aspect-[9/16] flex flex-col">
                <div className="p-5 flex items-center gap-3 border-b border-white/10">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white text-sm font-bold">K</div>
                  <div>
                ...
                <div className="p-4 space-y-3 flex-1">
                  {['Matemáticas', 'Física', 'Química', 'Historia'].map((t, i) => (
                    <div key={t} className={`p-3 rounded-xl border ${i === 0 ? 'border-cyan-400/40 bg-cyan-500/10' : 'border-white/10 bg-white/5'}`}>
                      <p className="text-xs font-semibold text-white mb-2">{t}</p>
                      <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                        <div className={`h-full rounded-full ${['w-4/5 bg-gradient-to-r from-cyan-400 to-primary-500', 'w-3/5 bg-gradient-to-r from-primary-500 to-accent-500', 'w-2/5 bg-gradient-to-r from-primary-500 to-accent-500', 'w-4/5 bg-gradient-to-r from-primary-500 to-accent-500'][i]}`} />
                      </div>
                    </div>
                  ))}
