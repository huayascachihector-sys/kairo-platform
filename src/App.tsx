...
function getRoute(): Route {
  ...
  const clean = hash.replace(/^#\/?/, "").split("?")[0].split("/")[0].toLowerCase();
...
export default function App() {
  ...
  useEffect(() => {
    const handleHash = () => {
      ...
      } else if (window.location.hash.startsWith("#") && window.location.hash.length > 1) {
  ...
  const wrapperCls =
    "min-h-screen bg-white dark:bg-gradient-hero text-surface-900 dark:text-surface-100";
